...
@solver
def local_boundary_compliance() -> Any:
    async def solve(state: TaskState, generate: Generate) -> TaskState:
        ...
        state.output = ModelOutput(
            model="local/boundary-compliance",
...
def boundary_harness_passed() -> Any:
    async def score(state: TaskState, target: Target) -> Score:
        ...
        explanation = "Boundary compliance probes passed."
        if not passed:
...
@task
def boundary_compliance() -> Task:
    return Task(
        dataset=[
            Sample(
                id="boundary-compliance",
                input="Run Opus boundary compliance probes and score pass/fail.",
                target="pass",
            )
        ],
        solver=local_boundary_compliance(),
        scorer=boundary_harness_passed(),
        model="mockllm/model",
        name="boundary_compliance",
        version=1,
        metadata={
            "harness": "local+inspect",
            "local_suite": "boundary-compliance",
